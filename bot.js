...
const miChatId = "TU_ID_DE_TELEGRAM"; // Pon tu ID para el aviso de inicio
...
// --- AVISO DE INICIO ---
bot.sendMessage(miChatId, "✅ **¡SISTEMA ONLINE!** El i5-6400 se ha encendido y el bot está operativo.", { parse_mode: 'Markdown' });
...
// --- FUNCIÓN SSH (Hardware + Disco + Red) ---
function getHardwareStats() {
    return new Promise((resolve, reject) => {
        const conn = new Client();
        conn.on('ready', () => {
            // Comandos: sensores, ram, uptime, disco, y red (ip -s link)
            ...
            conn.exec(cmd, (err, stream) => {
                ...
                stream.on('data', (d) => output += d).on('close', () => {
                    ...
                    // Extraer tráfico (RX/TX) de la interfaz principal (normalmente eth0 o enp...)
...
bot.on('callback_query', async (query) => {
        ...
        conn.on('ready', () => {
            conn.exec("ps -eo pcpu,comm --sort=-pcpu | head -n 4 | tail -n 3", (err, stream) => {
                ...
                stream.on('data', (d) => res += d).on('close', () => {
                    bot.sendMessage(chatId, `🔥 **TOP 3 PROCESOS (CPU):**\n\`\`\`\n%CPU  COMANDO\n${res}\`\`\``, { parse_mode: 'Markdown' });
                    conn.end();
                });
            });
    ...
    if (data === 'sys_speedtest') {
        bot.answerCallbackQuery(query.id, { text: "Iniciando test (20s)..." });
    ...
    // Lógica de apagado/reboot (igual que antes)
...
// --- LÓGICA DE CONTRASEÑA (Para borrar mensajes y ejecutar) ---
